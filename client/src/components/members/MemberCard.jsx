...
function MemberCard(props) {
  ...
  // NOTE: dummybutton used to standardize card sizes for members who have no social urls
  const dummybutton = (
    <IconButton disabled>
      <SvgIcon>
        <path />
  ...
  if (giturl) {
    gitbutton = (
      <IconButton onClick={() => window.open(giturl, '_blank').focus()}>
        <SvgIcon>
          <path d={logos.github} />
        </SvgIcon>
      </IconButton>
  ...
  if (linkedinurl) {
    libutton = (
      <IconButton onClick={() => window.open(linkedinurl, '_blank').focus()}>
        <SvgIcon>
  ...
  if (personalurl) {
    personalbutton = (
      <IconButton onClick={() => window.open(personalurl, '_blank').focus()}>
        <SvgIcon>
          ...
          <Grid container justify="space-between" alignItems="center">
            <Grid item>
              <Typography
                variant="h5"
                component="h2"
                style={{ ...styles.name, fontSize: `${scaleFontSize(name.length)}pt` }}
              >
                {name}
              </Typography>
            </Grid>
            <Grid item>
              <Typography variant="h5" component="h2" style={styles.year}>
                {year}
              </Typography>
            </Grid>
          </Grid>
